...
type SortOption = 'name' | 'age' | 'country' | 'farmCount' | 'farmLevel' | 'farmDiamonds';
...
export default function GestioneGiocatori() {
  ...
  const matchSearch = useCallback((text: string | undefined | null) => {
  ...
  const giocatoriFiltrati = useMemo(() => {
    ...
    const giocatoriConFarmAttive = risultati.filter(g => g.farms.some(f => f.stato === 'attivo'));
    const giocatoriSenzaFarmAttive = risultati.filter(g => g.farms.every(f => f.stato === 'inattivo'));
  ...
  const getFarmsFiltrate = useCallback((farms: Farm[]) => {
  ...
  const handleFarmChange = (index: number, field: keyof FormData['farms'][0], value: any) => {
  ...
  const handleSave = async () => {
    try {
      ...
      const cleanData = {
        ...
        dataRegistrazione: editingGiocatore ? editingGiocatore.dataRegistrazione : new Date().toISOString()
      };
  ...
  const [expandedPresentations, setExpandedPresentations] = useState<string[]>(() => {
    try {
      const saved = localStorage.getItem('gestioneGiocatori_expandedPresentations');
      return saved ? JSON.parse(saved) : [];
    } catch {
      return [];
    }
  ...
  const handleExpand = (giocatoreId: string) => {
    setExpandedPlayers(prev => {
      ...
      if (isExpanded) {
        ...
        setExpandedPresentations(prev => prev.filter(id => id !== giocatoreId));
  ...
  const handleUpdateFarmLevel = async (giocatore: Giocatore, farmIndex: number) => {
    try {
  ...
  const [filterMode, setFilterMode] = useState<FilterMode>(() => {
    try {
      return (localStorage.getItem('gestioneGiocatori_filterMode') as FilterMode) || 'players';
    } catch {
      return 'players';
    }
  });
  ...
  const [sortBy, setSortBy] = useState<SortOption>(() => {
    try {
      return (localStorage.getItem('gestioneGiocatori_sortBy') as SortOption) || 'name';
    } catch {
      return 'name';
    }
  });
  ...
  const [sortDirection, setSortDirection] = useState<'asc' | 'desc'>(() => {
    try {
      return (localStorage.getItem('gestioneGiocatori_sortDirection') as 'asc' | 'desc') || 'asc';
    } catch {
      return 'asc';
    }
  });
  ...
  const [filterFarmStatus, setFilterFarmStatus] = useState<'all' | 'active' | 'inactive'>(() => {
    try {
      return (localStorage.getItem('gestioneGiocatori_filterFarmStatus') as 'all' | 'active' | 'inactive') || 'all';
    } catch {
      return 'all';
    }
  });
  ...
  const [farmSortBy, setFarmSortBy] = useState<'farmName' | 'playerName' | 'level' | 'diamonds' | 'status'>(() => {
    try {
      return (localStorage.getItem('gestioneGiocatori_farmSortBy') as 'farmName' | 'playerName' | 'level' | 'diamonds' | 'status') || 'farmName';
    } catch {
      return 'farmName';
    }
  });
  ...
  const [farmSortDirection, setFarmSortDirection] = useState<'asc' | 'desc'>(() => {
    try {
      return (localStorage.getItem('gestioneGiocatori_farmSortDirection') as 'asc' | 'desc') || 'asc';
    } catch {
      return 'asc';
    }
  });
  ...
  useEffect(() => {
    localStorage.setItem('gestioneGiocatori_filterFarmStatus', filterFarmStatus);
  }, [filterFarmStatus]);
  ...
  useEffect(() => {
    localStorage.setItem('gestioneGiocatori_farmSortDirection', farmSortDirection);
  }, [farmSortDirection]);
  ...
  useEffect(() => {
    localStorage.setItem('gestioneGiocatori_expandedFilters', String(expandedFilters));
  }, [expandedFilters]);

  useEffect(() => {
    try {
      localStorage.setItem('gestioneGiocatori_expandedPlayers', JSON.stringify(expandedPlayers));
    } catch (error) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('gestioneGiocatori_expandedPresentations', JSON.stringify(expandedPresentations));
    } catch (error) {
      console.error('Errore nel salvataggio delle presentazioni espanse:', error);
    }
  }, [expandedPresentations]);

  // Nazioni principali
  const mainCountries = countries.filter(country => ['IT', 'GB', 'FR', 'ES'].includes(country.code));
  const otherCountries = countries.filter(country => !['IT', 'GB', 'FR', 'ES'].includes(country.code));
  ...
  const getCountryFlag = (code: string) => {
    ...
    return FlagComponent ? <FlagComponent style={{ width: '24px', marginRight: '8px' }} /> : <PeopleIcon sx={{ mr: 1 }} />;
  };

  // Funzione per gestire il click sulle intestazioni delle colonne
  const handleSortClick = (column: 'farmName' | 'playerName' | 'level' | 'diamonds' | 'status') => {
  ...
  const getSortIcon = (column: 'farmName' | 'playerName' | 'level' | 'diamonds' | 'status') => {
    if (farmSortBy !== column) return null;
    return farmSortDirection === 'asc' ? <ExpandMoreIcon /> : <ExpandLessIcon />;
  };
    ...
    if (filterMode === 'players') {
      ...
      if (filterFarmStatus !== 'all') {
        farmsFiltrate = farms.filter(farm => 
          filterFarmStatus === 'active' ? farm.status === 'attivo' : farm.status === 'inattivo'
        );
      }
      ...
      return farmsFiltrate.sort((a, b) => {
        ...
        switch (farmSortBy) {
          ...
          case 'diamonds':
            ...
            break;
          case 'status':
            comparison = (a.status === 'attivo' ? 1 : 0) - (b.status === 'attivo' ? 1 : 0);
            break;
        }
        return farmSortDirection === 'asc' ? comparison : -comparison;
      });
    }
  }, [giocatori, filterMode, sortBy, sortDirection, filterCountry, filterFarmStatus, 
      farmSortBy, farmSortDirection, searchQuery, matchSearch]);

  // Correzione degli errori di tipo
  const disabled = currentUser?.ruolo !== 'admin';

  const handleOpenMenu = (event: React.MouseEvent<HTMLElement>, giocatore: Giocatore) => {
  ...
  const handleToggleCoordinatore = async () => {
    ...
    try {
      const nuovoRuolo: Ruolo = selectedGiocatore.ruolo === 'coordinatore' ? 'giocatore' : 'coordinatore';
      ...
      setSuccess(`Giocatore ${nuovoRuolo === 'coordinatore' ? 'promosso a coordinatore' : 'degradato a giocatore'} con successo`);
      handleCloseMenu();
    } catch (error) {
  ...
  const handleToggleModeratore = async () => {
    if (!selectedGiocatore || !['admin', 'coordinatore'].includes(currentUser?.ruolo || '')) return;

    // Il coordinatore non può modificare altri coordinatori o moderatori
    if (currentUser?.ruolo === 'coordinatore' && 
        ['coordinatore', 'moderatore'].includes(selectedGiocatore.ruolo)) {
      return;
    }

    try {
      const nuovoRuolo: Ruolo = selectedGiocatore.ruolo === 'moderatore' ? 'giocatore' : 'moderatore';
      const giocatoreRef = doc(db, 'utenti', selectedGiocatore.id);
      ...
      setSuccess(`Giocatore ${nuovoRuolo === 'moderatore' ? 'promosso a moderatore' : 'degradato a giocatore'} con successo`);
      handleCloseMenu();
    } catch (error) {
      console.error('Errore nella modifica del ruolo:', error);
      setError('Errore nella modifica del ruolo');
  ...
  const handleToggleFarmStatus = async (giocatore: Giocatore, farmIndex: number) => {
    try {
      const nuovoStato: StatoFarm = giocatore.farms[farmIndex].stato === 'attivo' ? 'inattivo' : 'attivo';
      const farmsAggiornate = [...giocatore.farms];
      ...
      setSuccess(`Farm ${nuovoStato === 'attivo' ? 'attivata' : 'disattivata'} con successo`);
    } catch (error) {
    ...
    <Layout>
      <Box sx={{ maxWidth: '100%', overflow: 'hidden' }}>
        ...
        <Box sx={{ mb: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Button
              ...
              startIcon={expandedPlayers.length === giocatoriFiltrati.length ? <ExpandLessIcon /> : <ExpandMoreIcon />}
              size="small"
              sx={{ textTransform: 'none' }}
            >
              {expandedPlayers.length === giocatoriFiltrati.length ? 'Comprimi' : 'Espandi'}
            </Button>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, ml: 'auto' }}>
              ...
              {(() => {
                const farmAttive = giocatoriFiltrati.reduce((acc, g) => acc + g.farms.filter(f => f.stato === 'attivo').length, 0);
                return (
                  <Chip
                    ...
                    size="small"
                    color={
                      farmAttive === 0 ? 'default' :
                      farmAttive >= 30 ? 'success' :
                      'warning'
                    }
                  />
                );
              ...
              <Chip
                icon={<ErrorIcon />}
                label={giocatoriFiltrati.reduce((acc, g) => acc + g.farms.filter(f => f.stato === 'inattivo').length, 0)}
                color="default"
                size="small"
              />
            </Box>
                        ...
                        >
                          ...
                          {mainCountries.map((country) => (
                            <MenuItem key={country.code} value={country.code} sx={{ display: 'flex', alignItems: 'center' }}>
                              ...
                              {getCustomCountryName(country)} ({getCustomCountryCode(country.code)})
                            </MenuItem>
                          ))}
                          <MenuItem disabled sx={{ borderTop: '1px solid #ccc', my: 1, opacity: 0.7 }}>
                            ─────────────
                          </MenuItem>
                    ...
                    <Grid item xs={12}>
                      <FormControl fullWidth>
                        ...
                        <Select
                          ...
                          onChange={(e) => setFilterFarmStatus(e.target.value as 'all' | 'active' | 'inactive')}
                          label="Stato Farm"
                        >
        ...
        <Box sx={{ 
          ...
          {/* Rimuovo il pulsante ESPANDI duplicato */}
        </Box>
        ...
        {filterMode === 'players' ? (
          <Box sx={{ 
            ...
            {giocatoriFiltrati.map((giocatore, index) => {
              const hasActiveFarms = giocatore.farms.some(f => f.stato === 'attivo');
              const farmsFiltrate = getFarmsFiltrate(giocatore.farms);
              const mostraFarms = !searchQuery || matchSearch(giocatore.nome) ? giocatore.farms : farmsFiltrate;
              const isFirstInactive = !hasActiveFarms && giocatoriFiltrati[index - 1]?.farms.some(f => f.stato === 'attivo');

              return (
                <React.Fragment key={giocatore.id}>
                  {isFirstInactive && (
                    <Divider sx={{ my: 2, borderColor: 'divider', borderWidth: 1 }} />
                  )}
                  <Paper
                    elevation={1}
                    sx={{
                      ...
                      borderColor: 'divider',
                      bgcolor: !hasActiveFarms ? 'action.hover' : 'rgba(227, 242, 253, 0.7)',
                      '&:hover': {
                        bgcolor: !hasActiveFarms ? 'action.hover' : 'rgba(227, 242, 253, 0.9)'
                      }
                    }}
                  >
                    ...
                    >
                      ...
                      <Box sx={{ 
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                          ...
                          {['moderatore', 'coordinatore'].includes(giocatore.ruolo) && (
                            <Typography 
                              variant="caption"
                              sx={{ 
                                fontStyle: 'italic',
                      ...
                      <Box sx={{ 
                        ...
                        {/* Farm status */}
                        <Chip 
                          label={`${giocatore.farms.filter(f => f.stato === 'attivo').length}/${giocatore.farms.length}`}
                          size="small"
                          color={
                            giocatore.farms.every(f => f.stato === 'attivo') ? 'primary' :
                            giocatore.farms.some(f => f.stato === 'attivo') ? 'warning' :
                            'default'
                          }
                          sx={{ 
                            height: 24,
                            bgcolor: giocatore.farms.every(f => f.stato === 'attivo') ? 'primary.main' :
                                    giocatore.farms.some(f => f.stato === 'attivo') ? 'rgba(237, 108, 2, 0.7)' :
                                    undefined
                          }}
                        />

                        {/* Info Button */}
                        {(giocatore.note || giocatore.contatto || giocatore.eta || giocatore.nazionalita) && (
                          <IconButton
                            size="small"
                            onClick={(e) => {
                              e.stopPropagation();
                            ...
                            color={expandedPresentations.includes(giocatore.id) ? 'primary' : 'default'}
                            sx={{ p: 0.5 }}
                          >
                      ...
                      <Box sx={{ 
                        ...
                        {(canEditGiocatore(giocatore) || currentUser?.pin === giocatore.pin) && (
                    ...
                    {(giocatore.note || giocatore.contatto || giocatore.eta || giocatore.nazionalita || giocatore.pin) && (
                      <Collapse in={expandedPresentations.includes(giocatore.id)}>
                        <Box
                          sx={{
                            p: 1.5,
                            borderRadius: 1,
                        ...
                        >
                          ...
                          {/* Contatto */}
                          {(giocatore.contatto && (currentUser?.ruolo === 'admin' || currentUser?.pin === giocatore.pin || giocatore.contattoVisibile)) && (
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                              <Typography variant="subtitle2" color="text.primary">
                                Contatto:
                              </Typography>
                              ...
                              {!giocatore.contattoVisibile && currentUser?.ruolo !== 'admin' && currentUser?.pin === giocatore.pin && (
                    ...
                    <Box
                      sx={{
                        maxHeight: expandedPlayers.includes(giocatore.id) ? '1000px' : '0px',
                      ...
                      <Box sx={{ px: 2, pb: 2, bgcolor: 'background.paper' }}>
                        {mostraFarms.map((farm, farmIndex) => (
                          <Paper
                            ...
                            sx={{
                              p: 1,
                              bgcolor: farm.stato === 'attivo' ? '#ffffff' : 'action.disabledBackground',
                              position: 'relative',
                              borderRadius: 0,
                              mb: 0,
                              border: '1px solid',
                              ...
                              boxShadow: 'none',
                              '&:hover': {
                                bgcolor: farm.stato === 'attivo' ? 'rgba(255, 255, 255, 0.9)' : 'action.disabledBackground'
                              }
                            }}
                          >
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                              ...
                              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                                ...
                                <Box sx={{ 
                                  ...
                                  <Chip 
                                    size="small"
                                    label={farm.stato === 'attivo' ? 'Attiva' : 'Inattiva'}
                                    color={farm.stato === 'attivo' ? 'success' : 'default'}
                                    icon={<CircleIcon sx={{ fontSize: '12px !important' }} />}
                                    sx={{ 
                                      height: 20, 
                                      cursor: (currentUser?.ruolo === 'admin' || currentUser?.ruolo === 'coordinatore' || currentUser?.id === giocatore.id) ? 'pointer' : 'default'
                                    }}
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      // Permetti la modifica solo a admin, coordinatori o al proprietario
                                      if (currentUser?.ruolo === 'admin' || currentUser?.ruolo === 'coordinatore' || currentUser?.id === giocatore.id) {
                                  ...
                                  <Box sx={{ 
                                    ...
                                    />
                                    {(currentUser?.ruolo === 'admin' || currentUser?.ruolo === 'coordinatore' || currentUser?.pin === giocatore.pin) && (
                                      <IconButton
                                        size="small"
                                        onClick={(e) => {
                                          e.stopPropagation();
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                  {farm.tag && (
                                    <>
                                      <Box
                                        onClick={() => {
                                          navigator.clipboard.writeText(getCleanTag(farm.tag!));
                                  ...
                                  {farm.diamanti && (
                                    <Typography 
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        const newValue = prompt('Inserisci il nuovo valore dei diamanti:', farm.diamanti?.toString());
                                        if (newValue !== null) {
                                          ...
                                          if (!isNaN(numValue) && numValue >= 0) {
                                            ...
                                            setDoc(doc(db, 'utenti', giocatore.id), {
                                              ...giocatore,
                                              farms: farmsAggiornate
                                            }).then(() => {
                                              setSuccess('Diamanti aggiornati con successo!');
                                            }).catch((error) => {
                                              console.error('Errore nell\'aggiornamento dei diamanti:', error);
                                              setError('Errore nell\'aggiornamento dei diamanti');
                                            });
                                          } else {
                                            setError('Inserisci un numero valido maggiore o uguale a 0');
                                          }
                                        }
                                      }}
                                      sx={{ 
                ...
                {filteredItems.map((farm, index) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Typography
                        onClick={() => {
                          const giocatore = giocatori.find(g => g.id === farm.playerId);
                          if (giocatore && (
                            currentUser?.id === giocatore.id || 
                            ['admin', 'coordinatore', 'moderatore'].includes(currentUser?.ruolo || '')
                          )) {
                            navigator.clipboard.writeText(giocatore.pin.toString());
                            setSuccess('PIN copiato negli appunti!');
                          }
                        }}
                        sx={{ 
                          cursor: (currentUser?.id === farm.playerId || 
                                 ['admin', 'coordinatore', 'moderatore'].includes(currentUser?.ruolo || '')) 
                                 ? 'pointer' : 'default',
                          '&:hover': (currentUser?.id === farm.playerId || 
                                    ['admin', 'coordinatore', 'moderatore'].includes(currentUser?.ruolo || '')) 
                                    ? {
                            textDecoration: 'underline',
                            color: 'primary.main'
            ...
            <Grid container spacing={2} sx={{ mt: 1 }}>
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  disabled={!canEditGiocatore(editingGiocatore || { pin: '', ruolo: 'giocatore' } as Giocatore)}
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <Box sx={{ display: 'flex', gap: 1 }}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  onChange={(e) => {
                    ...
                    setFormData({ ...formData, eta: !isNaN(value) ? value.toString() : undefined });
                  }}
                  ...
                  <FormControlLabel
                    control={
                      <Switch
                        ...
                        onChange={(e) => setFormData({ ...formData, contattoVisibile: e.target.checked })}
                      />
                    }
                      ...
                      <Grid container spacing={2}>
                            ...
                            <FormControlLabel
                              control={
                                <Switch
                                  ...
                                  onChange={(e) => {
                                    const newFarms = formData.farms.map((f, i) => ({
                                      ...f,
                                      principale: i === index ? e.target.checked : false,
                                    }));
                        ...
                        <Grid item xs={12} sm={6}>
                          <TextField
                            ...
                            label="Nome Farm"
                            value={farm.nome}
                            onChange={(e) => handleFarmChange(index, 'nome', e.target.value)}
                          />
                        </Grid>
                        <Grid item xs={12} sm={6}>
                          <TextField
                            ...
                            onChange={(e) => {
                              ...
                              if (isValidTag(formattedTag)) {
                                ...
                                handleFarmChange(index, 'tag', value.toUpperCase());
                              }
                            }}
                            error={farm.tag !== '' && !isValidTag(farm.tag)}
                            helperText={farm.tag !== '' && !isValidTag(farm.tag) ? 
                        ...
                        <Grid item xs={12} sm={6}>
                          <TextField
                            ...
                            label="Livello"
                            value={farm.livello || ''}
                            onChange={(e) => handleFarmChange(index, 'livello', e.target.value ? parseInt(e.target.value) : undefined)}
                            margin="normal"
                        ...
                        <Grid item xs={12} sm={6}>
                          <TextField
                            ...
                            label="Diamanti"
                            value={farm.diamanti || ''}
                            onChange={(e) => handleFarmChange(index, 'diamanti', e.target.value ? parseInt(e.target.value) : undefined)}
                          />
                        </Grid>
                        ...
                        <Grid item xs={12}>
                          <FormControl fullWidth>
                            ...
                            <Select
                              ...
                              onChange={(e) => handleFarmChange(index, 'derby_tags', e.target.value)}
                            ...
                            >
                              {derby.map((d) => (
                                <MenuItem key={d.id} value={d.id}>
                                  <Checkbox checked={(farm.derby_tags || []).indexOf(d.id) > -1} />
                                  <Box
                                    component="span"
                                    sx={{
                                      width: 14,
                          ...
                          <FormControlLabel
                            control={
                              <Switch
                                ...
                                onChange={(e) => handleFarmChange(index, 'stato', e.target.checked ? 'attivo' : 'inattivo')}
                                disabled={!currentUser?.ruolo === 'admin'}
                              />
                            }
                            label={farm.stato === 'attivo' ? 'Farm Attiva' : 'Farm Inattiva'}
                          />
                        </Grid>
                      </Grid>
                    </CardContent>
                    {formData.farms.length > 1 && (
                      <CardActions>
                        <Button
                          ...
                          color="error"
                          onClick={() => {
                            const newFarms = formData.farms.filter((_, i) => i !== index);
                            setFormData({ ...formData, farms: newFarms });
                          }}
                        >
                          Rimuovi Farm
        ...
        >
          ...
          {(canEditGiocatore(selectedGiocatore) || currentUser?.pin === selectedGiocatore?.pin) && (
            <MenuItem onClick={() => {
          ...
          {currentUser?.ruolo === 'admin' && selectedGiocatore && selectedGiocatore.ruolo !== 'admin' && (
            <MenuItem onClick={handleToggleCoordinatore}>
              <ListItemIcon>
                <SupervisorAccountIcon fontSize="small" />
              </ListItemIcon>
              <ListItemText>
                {selectedGiocatore.ruolo === 'coordinatore' ? 'Rimuovi da coordinatore' : 'Promuovi a coordinatore'}
              </ListItemText>
            </MenuItem>
          )}
          ...
          {(['admin', 'coordinatore'].includes(currentUser?.ruolo || '') && 
            ...
            <MenuItem onClick={handleToggleModeratore}>
              ...
              <ListItemText>
                {selectedGiocatore.ruolo === 'moderatore' ? 'Rimuovi da moderatore' : 'Promuovi a moderatore'}
              </ListItemText>
            </MenuItem>
          )}
        ...
        <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
          ...
          <DialogContent>
            <Typography>
              Sei sicuro di voler eliminare il giocatore {selectedGiocatore?.nome}?
            </Typography>
          </DialogContent>
          <DialogActions>
