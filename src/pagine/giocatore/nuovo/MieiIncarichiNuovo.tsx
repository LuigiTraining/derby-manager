...
export default function MieiIncarichiNuovo() {
  ...
  const caricaAssegnazioniRef = useRef<(forzaAggiornamento?: boolean) => Promise<void>>(async () => {});
  ...
  const verificaModificheNonSincronizzateRef = useRef<(progressiLocaliAttuali: Map<string, number>) => void>(() => {});
  ...
  const caricaProgressiLocali = useCallback(() => {
    if (farmSelezionata) {
      try {
        ...
        const chiaveProgressi = `progressi_${farmSelezionata}`;
        const progressiSalvati = localStorage.getItem(chiaveProgressi);
        ...
        if (progressiSalvati) {
          ...
          // Nessun progresso locale trovato in localStorage per questa farm
        }

        // Carico anche gli incarichi modificati
        const chiaveModificati = `modificati_${farmSelezionata}`;
        const modificatiSalvati = localStorage.getItem(chiaveModificati);
        ...
        if (modificatiSalvati) {
          ...
          // Nessun incarico modificato trovato in localStorage per questa farm
        }
      } catch (error) {
        console.error("Errore nel caricamento dei progressi locali:", error);
      }
    } else {
      // Nessuna farm selezionata, cerco progressi modificati per qualsiasi farm
      const modificatiTotali = new Set<string>();

      // Cerco progressi modificati per tutte le farm
      for (let i = 0; i < 10; i++) { // Assumo max 10 farm
        const chiaveModificati = `modificati_${i}`;
        const modificatiSalvati = localStorage.getItem(chiaveModificati);
  ...
  }, [farmSelezionata, progressiLocali]);

  // Funzione per verificare se ci sono modifiche non sincronizzate
  ...
  const salvaProgressiLocali = useCallback(() => {
    try {
      ...
      // Stampa tutti i progressi locali salvati
      Object.entries(progressiObj).forEach(([key, value]) => {

      });
      
      // Stampa tutti gli incarichi modificati salvati
      progressiModificatiArray.forEach(id => {
  ...
  }, [farmSelezionata, caricaProgressiLocali]);
