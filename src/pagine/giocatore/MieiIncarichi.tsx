...
import { collection, query, where, getDocs, doc, getDoc, updateDoc, addDoc, deleteDoc, onSnapshot } from 'firebase/firestore';
...
export default function MieiIncarichi() {
  ...
  const [tuttiIncarichiCitta, setTuttiIncarichiCitta] = useState<IncaricoCitta[]>([]);
  ...
  // Funzioni di utilità
  const getQuantitaIncarico = useCallback((incaricoId: string): number => {
  ...
  const STORAGE_KEYS = {
    EXPANDED_EDIFICI: `miei-incarichi-expanded-edifici-${currentUser?.id || ''}-${farmSelezionata || ''}`,
    SCROLL_POSITION: `miei-incarichi-scroll-position-${currentUser?.id || ''}-${farmSelezionata || ''}`,
    MOSTRA_SOLO_ASSEGNATI: `miei-incarichi-mostra-assegnati-${currentUser?.id || ''}`, // Rimosso farmSelezionata
    FARM_SELEZIONATA: `miei-incarichi-farm-selezionata-${currentUser?.id || ''}`,
    CITTA_EXPANDED: `miei-incarichi-citta-expanded-${currentUser?.id || ''}-${farmSelezionata || ''}`,
    CESTI_EXPANDED: `miei-incarichi-cesti-expanded-${currentUser?.id || ''}-${farmSelezionata || ''}`,
    GIOCATORE_SELEZIONATO: `miei-incarichi-giocatore-selezionato-${currentUser?.id || ''}`
  };

  // Funzione per salvare lo stato degli edifici espansi
  const salvaStatoEdifici = useCallback((edifici: string[]) => {
    try {
      localStorage.setItem(STORAGE_KEYS.EXPANDED_EDIFICI, JSON.stringify(edifici));
    } catch (error) {
  ...
  const salvaMostraAssegnati = useCallback((mostra: boolean) => {
    try {
      localStorage.setItem(STORAGE_KEYS.MOSTRA_SOLO_ASSEGNATI, JSON.stringify(mostra));
    } catch (error) {
  ...
  const salvaFarmSelezionata = useCallback((farmId: string) => {
    try {
  ...
  const salvaCittaExpanded = useCallback((expanded: boolean) => {
    try {
      localStorage.setItem(STORAGE_KEYS.CITTA_EXPANDED, JSON.stringify(expanded));
    } catch (error) {
  ...
  const salvaCestiExpanded = useCallback((expanded: boolean) => {
    try {
      localStorage.setItem(STORAGE_KEYS.CESTI_EXPANDED, JSON.stringify(expanded));
    } catch (error) {
  ...
  }, [currentUser?.id, farmSelezionata, incarichi.length, STORAGE_KEYS.SCROLL_POSITION]);
  ...
  useEffect(() => {
    ...
    try {
      const savedMostraAssegnati = localStorage.getItem(STORAGE_KEYS.MOSTRA_SOLO_ASSEGNATI);
      if (savedMostraAssegnati !== null) {
        ...
        localStorage.setItem(STORAGE_KEYS.MOSTRA_SOLO_ASSEGNATI, JSON.stringify(true));
      }
    } catch (error) {
      console.error('Errore nel caricamento dello stato visualizzazione:', error);
  ...
  }, [currentUser?.id, farmSelezionata, mostraSoloAssegnati, salvaMostraAssegnati]);
  ...
  useEffect(() => {
    ...
    try {
      const savedCittaExpanded = localStorage.getItem(STORAGE_KEYS.CITTA_EXPANDED);
  ...
  useEffect(() => {
    ...
    try {
      const savedCestiExpanded = localStorage.getItem(STORAGE_KEYS.CESTI_EXPANDED);
  ...
  const getExpandButtonText = useMemo(() => {
    const tuttoEspanso = expandedEdifici.length === Object.keys(edifici).length && cestiEspansi;
  ...
  const caricaFarms = useCallback(async () => {
    ...
    try {
      const userId = (currentUser.ruolo === 'admin' || currentUser.ruolo === 'coordinatore' || currentUser.ruolo === 'moderatore') && giocatoreSelezionato ? 
      ...
      if (userDoc.exists()) {
        ...
        if (userData.farms) {
          const farmsData: Farm[] = userData.farms.map((farm: any, index: number) => ({
            id: `${userId}_${index}`,
  ...
  const caricaIncarichi = useCallback(async () => {
    try {
      ...
      const incarichiData = incarichiSnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      } as Incarico));
  ...
  const caricaAssegnazioni = useCallback(async () => {
    ...
    try {
      ...
      const assegnazioniData = assegnazioniSnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      } as Assegnazione));
  ...
  const caricaEdifici = useCallback(async () => {
    try {
      ...
      const edificiList = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }) as Edificio);
  ...
  const calcolaProgressoCesto = (cesto: Cesto): { completati: number; totale: number } => {
  ...
  const calcolaLivelloCesto = (cestoId: string): number => {
    ...
    incarichiCesto.forEach((incaricoInCesto) => {
      ...
      if (!incarico) {
        const incaricoCitta = incarichiCitta.find(i => i.id === incaricoInCesto.incarico_id);
  ...
  const aggiornaProgresso = async (incaricoId: string, nuovoValore: number) => {
    ...
    try {
      ...
      if (progressoDocId) {
        ...
        const nuovoProgressoRef = await addDoc(collection(db, 'progressi_incarichi'), {
          quantita: nuovoValore,
        ...
        // Aggiorniamo le mappe locali
        setProgressi(new Map(progressi.set(incaricoId, nuovoValore)));
        setProgressiDocs(new Map(progressiDocs.set(incaricoId, nuovoProgressoRef.id)));
      }
  ...
  const getQuantitaIncaricoCesto = useCallback((cestoId: string, incaricoId: string): number => {
  ...
  const getCompletamentiMultipli = (progresso: number, quantita: number): number => {
    return quantita > 0 ? Math.floor(progresso / quantita) : 0;
  };

  // Funzione per calcolare quante volte un cesto può essere completato
  const getCompletamentiCesto = useCallback((cestoId: string): number => {
    const cesto = cesti.find(c => c.id === cestoId);
    if (!cesto) return 0;
    ...
    const completamentiPerIncarico = cesto.incarichi.map(incaricoInCesto => {
      ...
      return Math.floor(progresso / getQuantitaIncaricoCesto(cestoId, incaricoInCesto.incarico_id));
    });
  ...
  const isCestoCompletato = useCallback((cestoId: string): boolean => {
    const cesto = cesti.find(c => c.id === cestoId);
    ...
    return cesto.incarichi.every(incaricoInCesto => {
      ...
      return progresso >= getQuantitaIncaricoCesto(cestoId, incaricoInCesto.incarico_id);
    });
  }, [getProgressoIncarico, getQuantitaIncaricoCesto]);
  ...
  const resetCesto = async (cestoId: string) => {
    ...
    try {
      ...
      const updates = cesto.incarichi.map(async incaricoInCesto => {
  ...
  const aggiornaProgressiMultipli = async (aggiornamenti: { incaricoId: string; nuovoValore: number; }[]) => {
    if (!farmSelezionata) return;

    try {
      const updates = await Promise.all(aggiornamenti.filter(Boolean).map(async ({ incaricoId, nuovoValore }) => {
        const progressoDocId = progressiDocs.get(incaricoId);
        ...
        if (progressoDocId) {
          if (nuovoValore === 0) {
            ...
            return updateDoc(doc(db, 'progressi_incarichi', progressoDocId), {
              quantita: nuovoValore
            });
          }
        } else if (nuovoValore > 0) {
  ...
  const matchSearch = useCallback((text: string | Incarico | undefined | null) => {
    if (!searchQuery) return true;
    if (!text) return false;
    const textToSearch = typeof text === 'string' ? text : text.nome;
  ...
  const completaCesto = async (cestoId: string) => {
    ...
    const aggiornamenti = cesto.incarichi
      .map(incaricoInCesto => {
        const progressoAttuale = getProgressoIncarico(incaricoInCesto.incarico_id);
        // Usa getQuantitaIncaricoCesto invece di incaricoInCesto.quantita
        const quantitaRichiesta = getQuantitaIncaricoCesto(cestoId, incaricoInCesto.incarico_id);
      ...
      .filter((update): update is { incaricoId: string; nuovoValore: number } => update !== null);
    ...
    if (incarico) {
      ...
      if (incarico.edificio_id) {
        setExpandedEdifici(prev => 
          prev.includes(incarico.edificio_id!) ? prev : [...prev, incarico.edificio_id!]
        );
      }
  ...
  const tuttiIncarichiNonDisponibili = useCallback((edificioId: string) => {
    const incarichiEdificio = incarichi.filter(inc => 
      inc.edificio_id === edificioId
    );
    return incarichiEdificio.length > 0 && incarichiEdificio.every(inc => !isIncaricoDisponibile(inc.livello_minimo));
  }, [incarichi, isIncaricoDisponibile]);

  // Funzione per verificare se un cesto ha incarichi assegnati
  const hasCestoIncarichiAssegnati = useCallback((cestoId: string) => {
    const incarichiCesto = cestiIncarichi.get(cestoId) || [];
  ...
  const hasContentToShow = useMemo(() => {
    ...
    const hasAssignedTasks = incarichi.some(inc => isIncaricoAssegnato(inc.id));
    const hasAssignedCityTasks = incarichiCitta.some(inc => isIncaricoAssegnato(inc.id));
    
    // Controlla se ci sono cesti con incarichi assegnati
    const hasAssignedBaskets = cesti.some(cesto => hasCestoIncarichiAssegnati(cesto.id));
    
    return hasAssignedTasks || hasAssignedCityTasks || hasAssignedBaskets;
  }, [incarichi, incarichiCitta, cesti, mostraSoloAssegnati, isIncaricoAssegnato, hasCestoIncarichiAssegnati]);

  // Funzione per verificare se un incarico corrisponde alla ricerca
  const matchesSearch = useCallback((incarico: Incarico) => {
  ...
  const edificiFiltrati = useMemo(() => {
    let edificiArray = Object.entries(edifici)
      .filter(([edificioId, nomeEdificio]) => {
        ...
        if (searchQuery) {
          const incarichiCorrispondenti = incarichi.filter(inc => 
            inc.edificio_id === edificioId && 
  ...
  const cestiFiltrati = useMemo(() => {
    return cesti.filter(cesto => {
      ...
      if (mostraSoloAssegnati) {
        ...
        const cestoAssegnato = assegnazioni.some(a => 
          a.tipo === 'cesto' && 
  ...
  }, [cesti, mostraSoloAssegnati, searchQuery, cestiIncarichi, incarichi, matchSearch, assegnazioni, farmSelezionata]);
  ...
  useEffect(() => {
    if (searchQuery) {
      ...
      const edificiDaEspandere = edificiFiltrati
        .filter(([edificioId]) => {
          const incarichiEdificio = incarichi.filter(inc => 
            inc.edificio_id === edificioId && matchesSearch(inc)
          );
  ...
  }, [searchQuery, edificiFiltrati, incarichi, matchesSearch, cestiFiltrati, cestiIncarichi]);
  ...
  }, [incarichi, ordinamento, getProgressoIncarico, getQuantitaIncarico]);
  ...
  const getIncarichiEdificio = useCallback((edificioId: string): Incarico[] => {
    ...
    if (mostraSoloAssegnati) {
      incarichi = incarichi.filter(incarico => {
        ...
        const haAssegnazioneDiretta = assegnazioni.some(a => 
          a.tipo === 'incarico' && 
        ...
        const isInCestoAssegnato = Array.from(cestiIncarichi.entries()).some(([cestoId, incarichiCesto]) => {
          const cestoAssegnato = assegnazioni.some(a => 
            a.tipo === 'cesto' && 
            a.riferimento_id === cestoId && 
            a.farm_id === farmSelezionata
          );
          return cestoAssegnato && incarichiCesto.some(inc => inc.incarico_id === incarico.id);
        });
    ...
    return incarichi;
  }, [incarichiPerEdificio, mostraSoloAssegnati, assegnazioni, farmSelezionata, cestiIncarichi, searchQuery]);

  // Modifica la funzione handleChangeFarm per accettare il nuovo tipo
  const handleChangeFarm = useCallback((event: React.SyntheticEvent, value: string) => {
  ...
  const trovaCestoPerIncarico = useCallback((incaricoId: string) => {
  ...
  const trovaNomeCestoPerIncarico = useCallback((incaricoId: string) => {
  ...
  const navigaACesto = useCallback((cestoId: string) => {
  ...
  const getCompletionColors = useCallback((incarico: Incarico) => {
    ...
    const quantitaTotaleCesti = cestiIncarichi ? 
      Array.from(cestiIncarichi.entries())
        .filter(([_, cesto]) => cesto.some(inc => inc.incarico_id === incarico.id))
        .reduce((total, [_, cesto]) => {
          const incaricoInCesto = cesto.find(inc => inc.incarico_id === incarico.id);
          return total + (incaricoInCesto ? getQuantitaIncaricoCesto(cesto.id, incarico.id) : 0);
        }, 0)
      : 0;
    ...
    if (quantitaProdotta >= quantitaTotaleNecessaria && quantitaTotaleNecessaria > 0) {
      return {
    ...
    else if (quantitaProdotta >= quantitaNecessaria && quantitaNecessaria > 0) {
      return {
  ...
  const calcolaQuantitaTotale = useCallback((incaricoId: string) => {
    ...
    const quantitaCesti = cestiIncarichi ? 
      Array.from(cestiIncarichi.entries())
        .filter(([cestoId, cesto]) => cesto.some(inc => inc.incarico_id === incaricoId))
        .reduce((total, [cestoId, cesto]) => {
          const incaricoInCesto = cesto.find(inc => inc.incarico_id === incaricoId);
          return total + (incaricoInCesto ? getQuantitaIncaricoCesto(cestoId, incaricoId) : 0);
        }, 0)
      : 0;
  ...
  const getChipStyle = useCallback((incaricoId: string, cestoId: string) => {
  ...
  const caricaGiocatori = useCallback(async () => {
    if (!['admin', 'coordinatore', 'moderatore'].includes(currentUser?.ruolo || '')) return;
  ...
  useEffect(() => {
    if (currentUser?.ruolo === 'admin' || currentUser?.ruolo === 'coordinatore' || currentUser?.ruolo === 'moderatore') {
      caricaGiocatori();
    }
  ...
  const getDatiStampa = useCallback(() => {
    ...
    const giocatoreCorrente = giocatori.find(g => g.id === (giocatoreSelezionato || currentUser?.id));

    // Oggetto per raggruppare gli incarichi per edificio
    const incarichiPerEdificio: { [key: string]: Array<{
      incarico: Incarico;
      quantita: number;
      cesto?: Cesto;
    }> } = {};
    ...
    cesti.forEach(cesto => {
      const cestoAssegnato = assegnazioni.some(a => a.tipo === 'cesto' && a.riferimento_id === cesto.id);
      if (cestoAssegnato) {
        cesto.incarichi.forEach(incaricoInCesto => {
  ...
  }, [incarichi, assegnazioni, cesti, currentUser, isIncaricoAssegnato, farms, farmSelezionata, giocatori, giocatoreSelezionato, incarichiCitta, getQuantitaIncaricoCitta, totaleCitta, derbySelezionato]);
  ...
  const caricaIncarichiCitta = async () => {
    try {
      ...
      const incarichiCittaSnapshot = await getDocs(collection(db, 'incarichi_citta'));
      const incarichiCittaData = incarichiCittaSnapshot.docs.map(doc => ({ 
        id: doc.id, 
        ...doc.data() 
      } as IncaricoCitta));
      ...
      if (mostraSoloAssegnati) {
        ...
        const incarichiAssegnati = assegnazioniSnapshot.docs
          .map(doc => {
            ...
            return incarichiCittaData.find(inc => inc.id === assegnazione.riferimento_id);
          })
  ...
  const calcolaTotaleProgressiCitta = useCallback(async (): Promise<number> => {
    try {
      ...
      const incarichiCittaSnapshot = await getDocs(collection(db, 'incarichi_citta'));
      const tuttiIncarichi = incarichiCittaSnapshot.docs.map(doc => ({ 
        id: doc.id, 
        ...doc.data() 
      } as IncaricoCitta));
  ...
  const salvaGiocatoreSelezionato = useCallback((giocatoreId: string | null) => {
    try {
      ...
      console.error('Errore nel salvataggio del giocatore selezionato:', error);
    }
  ...
  useEffect(() => {
    ...
    try {
      const savedGiocatore = localStorage.getItem(STORAGE_KEYS.GIOCATORE_SELEZIONATO);
      if (savedGiocatore) {
  ...
  const renderIncaricoCard = (incarico: Incarico) => {
    ...
    const completamenti = getCompletamentiMultipli(progresso, getQuantitaIncarico(incarico.id));
        ...
        <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Box sx={{ 
              ...
              <Checkbox
                ...
                color={getCompletionColors(incarico).checkboxColor as 'success' | 'warning' | 'default'}
                sx={{ p: 0.5 }}
              />
              {completamenti > 0 && (
                <Typography
          ...
          <Box sx={{ 
            ...
            <Stack spacing={0.5} sx={{ mt: 0.5 }}>
              ...
              {cestiIncarichi && Array.from(cestiIncarichi.entries()).some(([_, cesto]) =>
                cesto.some(inc => inc.incarico_id === incarico.id)
              ) && (
                <Stack spacing={0.5}>
                  {cestiIncarichi && Array.from(cestiIncarichi.entries()).map(([cestoId, cesto]) => {
                    const cestoIncarico = cesto.find(inc => inc.incarico_id === incarico.id);
                    ...
                    return (
                      <Chip 
                        key={cestoId}
                        size="small" 
                        label={`${cestoInfo.nome} (${quantitaProdotta >= quantitaCesto ? quantitaCesto : quantitaProdotta}/${quantitaCesto})`}
                        sx={{ 
                          height: '20px',
                          cursor: 'pointer',
                          width: 'fit-content',
                  ...
                  {cestiIncarichi && Array.from(cestiIncarichi.entries()).some(([_, cesto]) => 
                    ...
                    <Typography 
                      ...
                      sx={{ 
                        color: (theme) => {
                          ...
                          const prodotto = getProgressoIncarico(incarico.id);
                    ...
                    >
                      Tot: {getProgressoIncarico(incarico.id)}/{calcolaQuantitaTotale(incarico.id).totale}
                    </Typography>
                  )}
                </Stack>
              )}
          ...
          <Box sx={{ 
            ...
            <IconButton
              ...
              onClick={() => aggiornaProgresso(incarico.id, Math.max(0, progresso - 1))}
              sx={{ p: '2px' }}
            >
            ...
            <TextField
              ...
              sx={{
                ...
                '& .MuiOutlinedInput-root': {
                  ...
                  '&:hover fieldset': {
                    borderColor: isDisponibile ? 'rgba(0, 0, 0, 0.23)' : 'transparent'
                  },
  ...
  return (
    <Layout>
      <Box sx={{ p: { xs: 1, sm: 2 } }}>
        ...
        {(currentUser?.ruolo === 'admin' || currentUser?.ruolo === 'coordinatore' || currentUser?.ruolo === 'moderatore') && (
              <Grid item xs={12}>
            <FormControl 
              fullWidth
                  size="small"
            ...
            >
              ...
              <Select
                ...
                onChange={(e) => {
                  const newGiocatore = e.target.value === '' ? null : e.target.value;
                ...
                label="Seleziona Giocatore"
                    renderValue={(selected) => {
                      if (!selected) {
                        const farmCount = currentUser?.farms?.filter(f => f.stato === 'attivo').length || 0;
                      ...
                      const giocatore = giocatori.find(g => g.id === selected);
                      if (!giocatore) return '';
                      const farmCount = giocatore.farms.filter(f => f.stato === 'attivo').length;
                      return (
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              >
                    ...
                    >
                      ...
                      >
                        ({currentUser?.farms?.filter(f => f.stato === 'attivo').length || 0}/{currentUser?.farms?.length || 0})
                      </Typography>
                </MenuItem>
                    ...
                    {giocatori
                      ...
                      .map((g) => (
                        ...
                        >
                          ...
                          >
                            ({g.farms.filter(f => f.stato === 'attivo').length}/{g.farms.length})
                          </Typography>
                        </MenuItem>
                    ))}
                    ...
                    {giocatori
                      .filter(g => g.farms.every(f => f.stato === 'inattivo'))
                      .sort((a, b) => a.nome.localeCompare(b.nome))
                      .map((g) => (
                        <MenuItem 
                          key={g.id} 
                    ...
                    <Box sx={{ textAlign: 'center' }}>
                      <Typography 
                        sx={{ 
                          fontSize: '0.875rem', 
                          lineHeight: 1.2,
                          fontWeight: farmSelezionata === farm.farmId ? 700 : 400
                        }}
                      >
                        {farm.nome}
                      </Typography>
        ...
        <Box sx={{ 
          ...
          <Box sx={{ 
            ...
            <Box sx={{
              ...
              <Button
                startIcon={expandedEdifici.length === Object.keys(edifici).length && cestiEspansi && cittaExpanded ? 
                  <ExpandLessIcon /> : <ExpandMoreIcon />}
                onClick={() => {
                  if (expandedEdifici.length === Object.keys(edifici).length && cestiEspansi && cittaExpanded) {
                    // Comprimi tutto
                    setExpandedEdifici([]);
                    setCestiEspansi(false);
              ...
              >
                {expandedEdifici.length === Object.keys(edifici).length && cestiEspansi && cittaExpanded ? 
                  'COMPRIMI' : 'ESPANDI'}
              </Button>
        ...
        {(!mostraSoloAssegnati || (mostraSoloAssegnati && incarichiCitta.length > 0)) && 
         (!searchQuery || hasIncarichiCittaResults) && (
          <Box sx={{ mb: 0 }}> {/* Ridotto da mb: 2 a mb: 0 */}
            <Box 
              sx={{ 
                display: 'flex', 
                alignItems: 'center',
              ...
              <Box sx={{ 
                ...
                <Typography
                  sx={{
                    ...
                    fontWeight: 'medium',
                    color: (theme) => {
                      if (totaleCitta === SOGLIA_MASSIMA_CITTA) return theme.palette.success.main;
                      if (totaleCitta > SOGLIA_MASSIMA_CITTA) return theme.palette.error.main;
                ...
                >
                  {Math.min(totaleCitta, SOGLIA_MASSIMA_CITTA)}/{SOGLIA_MASSIMA_CITTA}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  {incarichiCitta.length} {incarichiCitta.length === 1 ? 'incarico' : 'incarichi'}
                </Typography>
                <IconButton 
                  size="small"
                  onClick={(e) => {
                  ...
                  sx={{
                    ...
                    transform: cittaExpanded ? 'rotate(180deg)' : 'rotate(0deg)'
                  }}
                >
                  <ExpandMoreIcon />
                </IconButton>
            ...
            <Collapse in={cittaExpanded}>
              <Box sx={{ mb: 3 }}>
                {incarichiCitta.length > 0 ? (
                  incarichiCittaFiltrati.map(incarico => renderIncaricoCard(incarico))
                ) : (
                  <Typography 
                    variant="body2" 
                    color="text.secondary"
        ...
        {hasCestiResults && (!mostraSoloAssegnati || (mostraSoloAssegnati && cesti.some(cesto => isCestoAssegnato(cesto.id)))) && (
          <>
            <Box sx={{ 
              display: 'flex', 
              alignItems: 'center',
              ...
              <Box sx={{ 
                ...
                alignItems: 'center', 
                gap: 1,
                '@media (max-width: 600px)': { // Responsive per mobile
                  '& .MuiTypography-root': {
                    fontSize: '0.875rem'
                  }
                }
                ...
                <Typography variant="body2" color="text.secondary">
                  {mostraSoloAssegnati 
                    ? `${cestiFiltrati.filter(cesto => isCestoAssegnato(cesto.id)).length} cesti`
                    : `${cesti.length} cesti`
                  }
                </Typography>
                <IconButton 
                  ...
                  sx={{
                    transition: 'transform 0.2s ease-in-out',
                    transform: cestiEspansi ? 'rotate(180deg)' : 'rotate(0deg)'
                  }}
                >
                  <ExpandMoreIcon />
                </IconButton>
            ...
            <Collapse in={cestiEspansi} timeout={200}>
              <Box sx={{ mb: 3 }}>
                {cestiFiltrati
                  ...
                  const haIncarichiNonDisponibili = incarichiCesto.some(inc => {
                    const incarico = incarichi.find(i => i.id === inc.incarico_id);
                    return incarico && !isIncaricoDisponibile(incarico.livello_minimo);
                  });
                  const tuttiIncarichiNonDisponibili = incarichiCesto.every(inc => {
                    const incarico = incarichi.find(i => i.id === inc.incarico_id);
                    return incarico && !isIncaricoDisponibile(incarico.livello_minimo);
                  });
                  
                  return (
                    <TableContainer key={cesto.id} id={`cesto-${cesto.id}`} component={Paper} sx={{ 
                      position: 'relative',
                      ...
                      ...(!isCestoCompletato(cesto.id) && haIncarichiNonDisponibili && {
                        bgcolor: 'grey.100',
                        '& .MuiTypography-root': {
                          color: 'text.disabled'
                        },
                      ...
                      ...(!isCestoCompletato(cesto.id) && !haIncarichiNonDisponibili && isCestoAssegnato(cesto.id) && {
                      ...
                      <Table>
                              ...
                              <Box sx={{ 
                                ...
                                <Box sx={{ 
                                  ...
                                  <Checkbox
                                    ...
                                    disabled={tuttiIncarichiNonDisponibili || !isIncaricoDisponibile(calcolaLivelloCesto(cesto.id))}
                                    onChange={(e) => {
                                      if (e.target.checked) {
                                ...
                                <Box sx={{ 
                                  ...
                                  {getCompletamentiCesto(cesto.id) > 0 && (
                                    <Chip
                                      size="small"
                                      label={`x${getCompletamentiCesto(cesto.id)}`}
                                      color="success"
                                      sx={{ height: '20px' }}
                                    />
                                  )}
                                ...
                                <Box sx={{ 
                                  ...
                                  {cestiIncarichi.get(cesto.id)?.map((incaricoInCesto) => {
                                    // Cerca prima tra gli incarichi standard
                                    let incarico = incarichi.find(i => i.id === incaricoInCesto.incarico_id);
                                    
                                    // Se non lo trova, cerca tra gli incarichi città
                                    if (!incarico) {
                                      const incaricoInCitta = incarichiCitta.find(i => i.id === incaricoInCesto.incarico_id);
                                      if (incaricoInCitta) {
                                        incarico = {
                                    ...
                                    const isDisponibile = isIncaricoDisponibile(incarico.livello_minimo);
                                    const progresso = getProgressoIncarico(incarico.id);
                                    const quantitaCesto = getQuantitaIncaricoCesto(cesto.id, incarico.id);
                        ...
                        alignItems: 'center',
                        gap: 0.5,
                                          filter: isDisponibile ? 'none' : 'grayscale(100%)',
                                      ...
                                      >
                                        <Avatar
                                          ...
                                          onClick={() => scrollToIncarico(incarico.id)}
                                        >
                                          {incarico.nome.charAt(0)}
                                        </Avatar>
                                        <Typography 
                                          variant="caption" 
                                          sx={{ 
                                            color: progresso >= quantitaCesto ? 'success.main' : 'text.secondary',
                                            fontWeight: 'medium',
                                            fontSize: '0.7rem',
                                            lineHeight: 1
                                          }}
        ...
        <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <IconButton
              ...
              size="small"
              title={visualizzazioneGlobale ? "Vista lista" : "Vista per edificio"}
            >
            ...
            <Box>
              ...
              >
                ...
                <MenuItem 
                  ...
                  sx={{ 
                    color: ordinamento.tipo === 'alfabetico' && ordinamento.direzione === 'asc' ? 'primary.main' : 'inherit',
                    minWidth: '200px'
                  }}
                >
                  <ListItemIcon>
                    <ArrowUpwardIcon sx={{ color: ordinamento.tipo === 'alfabetico' && ordinamento.direzione === 'asc' ? 'primary.main' : 'inherit' }} />
                  </ListItemIcon>
                ...
                <MenuItem 
                  ...
                  sx={{ 
                    color: ordinamento.tipo === 'alfabetico' && ordinamento.direzione === 'desc' ? 'primary.main' : 'inherit'
                  }}
                >
                  <ListItemIcon>
                    <ArrowDownwardIcon sx={{ color: ordinamento.tipo === 'alfabetico' && ordinamento.direzione === 'desc' ? 'primary.main' : 'inherit' }} />
                  </ListItemIcon>
                ...
                <MenuItem 
                  ...
                  sx={{ 
                    color: ordinamento.tipo === 'livello' && ordinamento.direzione === 'asc' ? 'primary.main' : 'inherit'
                  }}
                >
                  <ListItemIcon>
                    <ArrowUpwardIcon sx={{ color: ordinamento.tipo === 'livello' && ordinamento.direzione === 'asc' ? 'primary.main' : 'inherit' }} />
                  </ListItemIcon>
                ...
                <MenuItem 
                  ...
                  sx={{ 
                    color: ordinamento.tipo === 'livello' && ordinamento.direzione === 'desc' ? 'primary.main' : 'inherit'
                  }}
                >
                  <ListItemIcon>
                    <ArrowDownwardIcon sx={{ color: ordinamento.tipo === 'livello' && ordinamento.direzione === 'desc' ? 'primary.main' : 'inherit' }} />
                  </ListItemIcon>
                ...
                <MenuItem 
                  ...
                  sx={{ 
                    color: ordinamento.tipo === 'completamento' && ordinamento.direzione === 'asc' ? 'primary.main' : 'inherit'
                  }}
                >
                  <ListItemIcon>
                    <ArrowUpwardIcon sx={{ color: ordinamento.tipo === 'completamento' && ordinamento.direzione === 'asc' ? 'primary.main' : 'inherit' }} />
                  </ListItemIcon>
                  <ListItemText>Da completare</ListItemText>
                </MenuItem>
                <MenuItem 
                  onClick={() => {
                    setOrdinamento({ tipo: 'completamento', direzione: 'desc' });
                    setAnchorElSort(null);
                  }}
                  sx={{ 
                    color: ordinamento.tipo === 'completamento' && ordinamento.direzione === 'desc' ? 'primary.main' : 'inherit'
                  }}
                >
                  <ListItemIcon>
                    <ArrowDownwardIcon sx={{ color: ordinamento.tipo === 'completamento' && ordinamento.direzione === 'desc' ? 'primary.main' : 'inherit' }} />
                  </ListItemIcon>
                ...
                <MenuItem 
                  ...
                  sx={{ 
                    color: ordinamento.tipo === null ? 'primary.main' : 'inherit'
                  }}
                >
                  <ListItemIcon>
                    <RestartAltIcon sx={{ color: ordinamento.tipo === null ? 'primary.main' : 'inherit' }} />
                  </ListItemIcon>
        ...
        <Box>
          ...
          <Box>
            {visualizzazioneGlobale ? (
              // Vista lista globale
              <Box>
                {incarichiDaMostrare.map((incarico) => renderIncaricoCard(incarico))}
              </Box>
            ) : (
              ...
              <Box>
                {edificiFiltrati.map(([edificioId, nomeEdificio]) => {
                  ...
                  const tuttiNonDisponibili = tuttiIncarichiNonDisponibili(edificioId);

                  if (incarichiEdificio.length === 0) return null;

                  const isExpanded = expandedEdifici.includes(edificioId);
                  const edificioData = edificiData.find(e => e.id === edificioId);
                  ...
                  return (
                    <Box key={edificioId}>
                      ...
                      <Collapse in={isExpanded}>
                        <Box>
                          {incarichiEdificio.map((incarico) => renderIncaricoCard(incarico))}
                        </Box>
                      </Collapse>
                    </Box>
                  );
