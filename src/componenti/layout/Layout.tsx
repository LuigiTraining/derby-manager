  ...
  const getMenuItems = () => {
    ...
    if (['admin', 'coordinatore'].includes(currentUser.ruolo)) {
      const items: MenuItem[] = [
        { 
          text: 'Home', 
          icon: (
            <Badge badgeContent={nuoviAnnunci.length} color="error" variant="dot">
              <HomeIcon />
            </Badge>
          ),
          children: [
            { text: 'Annunci', icon: <AnnouncementIcon />, path: '/welcome' },
            { text: 'Wiki', icon: <MenuBookIcon />, path: '/wiki/home' },
            { text: 'Tutorial', icon: <SchoolIcon />, path: '/tutorial/home' },
            { text: 'Regolamenti', icon: <GavelIcon />, path: '/regolamento/home' },
          ]
        },
        { text: 'Dashboard', icon: <DashboardIcon />, path: '/dashboard' },
        { text: 'Assegnazioni', icon: <AssignmentIcon />, path: '/admin/assegnazioni' },
        { text: currentUser.ruolo === 'admin' ? 'Lista globale' : 'Lista', icon: <TaskIcon />, path: '/miei-incarichi' },
        { text: 'Blocchi', icon: <BlockIcon />, path: '/admin/blocchi' },
        { text: 'Giocatori', icon: <PeopleIcon />, path: '/giocatori' },
      ];

      // Creiamo il menu Gestione
      const gestioneMenu: MenuItem = {
        text: 'Gestione',
        icon: <BuildIcon />,
        children: [
          ...gestioneItems,
          // Aggiungiamo Derby e Statistiche per admin e coordinatori
          ...((['admin', 'coordinatore'].includes(currentUser.ruolo)) ? [
            { text: 'Derby', icon: <EmojiEventsIcon />, path: '/admin/derby' },
            { text: 'Statistiche', icon: <BarChartIcon />, path: '/admin/statistiche' }
          ] : []),
        ]
      };

      items.push(gestioneMenu);

      // Aggiungiamo le impostazioni
      items.push({ 
        text: 'Impostazioni', 
        icon: (
          <Badge badgeContent={nuoveRichieste} color="error" variant="dot">
            <SettingsIcon />
          </Badge>
        ), 
        path: '/impostazioni' 
    ...
    if (currentUser.ruolo === 'moderatore') {
      return [
        ...
        { text: 'Dashboard', icon: <DashboardIcon />, path: '/dashboard' },
        { text: 'Assegnazioni', icon: <AssignmentIcon />, path: '/admin/assegnazioni' },
        { text: 'Lista', icon: <TaskIcon />, path: '/miei-incarichi' },
        { text: 'Giocatori', icon: <PeopleIcon />, path: '/giocatori' },
        {
          text: 'Gestione',
          icon: <BuildIcon />,
      ...
      >
        <Toolbar>
          ...
          <Box sx={{ textAlign: 'right' }}>
            ...
            {currentUser && ['moderatore', 'coordinatore'].includes(currentUser.ruolo) && (
              <Typography 
                variant="caption" 
                sx={{ 
                  color: 'white',
